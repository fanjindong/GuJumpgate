...
> [!IMPORTANT]
> 侧边栏中的 `Plus 模式` 已固定开启。`账户 JSON` 支持条件必填：填写后会强制按该账号重新登录；留空时只在当前窗口已检测到 ChatGPT 登录态后，才会复用当前会话直接进入 Plus 订阅链路。
>
...
- 希望在 Chrome 扩展侧边栏中按真实页面观察流程状态、日志和账号运行记录。
...
可以从本仓库的 [Releases](https://github.com/fanjindong/GuJumpgate/releases) 下载压缩包并解压，也可以直接使用本仓库根目录作为未打包扩展目录。
...
侧边栏第一项是 `账户 JSON`。它有两种使用方式：

- **指定账号模式**：填写 `账户 JSON`。点击 `自动` 后，扩展会清理 ChatGPT/OpenAI 相关 cookies，并使用 JSON 中的账号重新登录。这个模式适合批量跑账号，或从第一个账号切换到第二个账号。
- **复用当前登录态模式**：留空 `账户 JSON`。点击 `自动` 后，扩展会检测当前窗口中的 ChatGPT 页面；只有读取到有效 `accessToken`，才会跳过登录节点并直接创建 Plus Checkout。此模式只适合立即启动，不适合定时启动。
...
> [!NOTE]
> `账户 JSON` 非空时优先级最高。即使浏览器里已经登录了其他 ChatGPT 账号，扩展也会重新登录 JSON 指定账号，避免把上一个账号的登录态误用于当前账号。
...
启动前必须满足下面任意一种 PayPal Hosted 接码配置：

- 填写 `验证码接口`，并填写 `PayPal 电话(不带+1)`。
- 导入 `Hosted 接码池`，池内每条记录需要同时包含号码和验证码接口。

PayPal 默认步骤：

侧边栏主流程按浏览器页面展示，而不是直接展示内部节点：

1. `ChatGPT 页面`：打开 ChatGPT 官网并识别会话。
2. `认证页`：登录已有账户并处理登录验证码。
3. `Checkout 页面`：创建 Plus Checkout，并填写 Hosted Checkout。
4. `PayPal 页面`：处理 PayPal Hosted 支付。
5. `开通结果`：确认 Plus 开通成功。

展开页面行可以查看底层节点状态，便于排查异常。

## 开始自动运行

1. 选择账号来源：
   - 要跑指定账号：填写并保存 `账户 JSON`。
   - 要复用当前登录态：留空 `账户 JSON`，并确认当前窗口已有已登录的 ChatGPT 页面。
2. 确认 `Plus 支付` 为 `PayPal`。
3. 补齐 PayPal Hosted 相关配置：`验证码接口 + PayPal 电话(不带+1)`，或 `Hosted 接码池`。
4. 确认当前浏览器处于全局美国节点环境。
...
- `账户 JSON` 非空：自动流程从 ChatGPT 页面重新开始，清理旧 cookies，并登录 JSON 中的账号。
- `账户 JSON` 为空：后台会读取当前窗口 ChatGPT 页面的 `/api/auth/session`；只有拿到 `accessToken` 才会跳过登录并继续 Checkout。
- 定时自动运行必须填写 `账户 JSON`。复用当前登录态只支持立即启动，避免倒计时期间登录态变化导致账号误用。

运行过程中，侧边栏会显示当前页面步骤、日志、失败原因和账号记录。点击停止后，旧倒计时、旧恢复入口和旧重试任务不会重新拉起已经停止的自动流程。

## 异常恢复

流程中断后，侧边栏会请求后台重新检测当前标签页和节点状态，并在流程上方显示恢复建议。常见恢复入口包括：

- 当前已经进入 ChatGPT 登录态时，跳过认证页并从 Checkout 页面继续。
- 当前停留在认证验证码页时，继续获取登录验证码，不重新提交邮箱和密码。
- 当前标签页是 Hosted Checkout 时，接管该页面继续填写。
- 当前标签页是 PayPal Hosted 支付页时，接管该页面继续支付。

恢复动作必须由用户点击确认后才会执行。后台会在执行前再次校验当前标签页 URL，避免用户切换页面后误接管。
...
如果你打算复用当前已经登录的 ChatGPT 页面，可以留空 `账户 JSON` 后直接点击 `自动`；如果后台没有检测到有效 `accessToken`，会提示先手动登录或填写 `账户 JSON`。

### 启动后为什么会重新登录？

只要 `账户 JSON` 非空，扩展就认为你指定了目标账号，会清理旧登录态并重新登录该账号。这是为了避免浏览器里已有的上一个账号被误用于当前流程。

### 复用当前登录态为什么失败？

当前窗口必须存在已登录的 ChatGPT 页面，并且扩展能从该页面读取到 `/api/auth/session` 中的 `accessToken`。如果只是打开了 `https://chatgpt.com/` 但未登录，或页面会话已失效，扩展不会跳过认证。

### 点击自动后浏览器窗口被关闭

请先重新加载扩展到最新版本。旧版本在重新登录指定账号时，可能会先关闭旧 `chatgpt.com` 标签页再创建新标签页；如果旧标签页正好是窗口里的最后一个标签，Chrome 会关闭整个窗口。当前版本已经改为先创建新自动化标签，再清理旧冲突标签，避免这个问题。
